...
<template>
  <div class="space-y-4">
    <div class="flex flex-wrap items-end justify-between gap-3">
      <div>
        <h2 class="text-sm font-semibold text-highlighted">{{ t("docgen.picker.title") }}</h2>
        <p class="text-xs text-muted">{{ t("docgen.picker.subtitle") }}</p>
      </div>
    ...
    <div v-if="props.loading" class="flex items-center gap-2 py-6 text-sm text-muted">
      <UIcon name="i-tabler-loader-2" class="size-4 animate-spin" />
      {{ t("docgen.loading") }}
    </div>

    <div v-else-if="props.templates.length === 0" class="py-6 text-sm text-muted">
    ...
    <div v-else class="grid gap-3 sm:grid-cols-2">
      ...
      >
        <div class="flex items-start justify-between gap-2">
          ...
          <UBadge color="neutral" variant="subtle" size="sm" :label="`v${template.version}`" />
        </div>
        <p class="mt-1 text-xs text-muted">{{ localized(template.description) }}</p>
    ...
    <div class="space-y-2">
      <p class="text-xs font-medium text-toned">{{ t("docgen.picker.mode") }}</p>
